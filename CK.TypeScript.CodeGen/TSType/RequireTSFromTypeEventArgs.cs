                               ...
                               Type? sameFileAs )
    {
        ...
        if( folder != null )
        {
            ...
            if( folder.Length > 0 )
            {
                ...
                var e = TypeScriptFolder.GetInvalidFileOrFolderNameError( folder, isFolder: true );
                if( e != null )
                {
        ...
        if( fileName != null )
        {
            ...
            {
                monitor.Error( $"If FileName is not null it must be not empty or whitespace." );
                success = false;
            }
            var e = TypeScriptFolder.GetInvalidFileOrFolderNameError( fileName, isFolder: false );
            if( e != null )
            {
