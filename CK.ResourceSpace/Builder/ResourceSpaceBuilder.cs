...
{
    readonly List<ResourceSpaceHandler> _resourceHandlers;
    ...
    public ResourceSpaceBuilder()
    {
        ...
        _resourceHandlers = new List<ResourceSpaceHandler>();
    }

    public IReadOnlyList<ResourceSpaceHandler> ResourceHandlers => _resourceHandlers;

    /// <summary>
    /// Adds a <see cref="ResourceHandlers"/>. Folder based handlers have priority over the
    /// ones that use file extensions.
    /// <para>
    /// If a handler with the same <see cref="ResourceSpaceHandler.RootFolderName"/> or with
    /// a common <see cref="ResourceSpaceHandler.FileExtensions"/> already exists, this is an error.
    /// </para>
    /// </summary>
    /// <param name="monitor">The monitor to use.</param>
    /// <param name="h">The handler to register.</param>
    /// <returns>True on success, false on error.</returns>
    public bool RegisterHandler( IActivityMonitor monitor, ResourceSpaceHandler h )
    {
        Throw.CheckNotNullArgument( h );
        if( _resourceHandlers.Contains( h ) )
        {
            monitor.Warn( $"Duplicate handler registration for '{h}'. Ignored." );
            return true;
        }
        if( h.RootFolderName != null )
        {
            var conflict = _resourceHandlers.Where( existing => h.RootFolderName.Equals( existing.RootFolderName, StringComparison.OrdinalIgnoreCase ) );
            if( conflict.Any() )
            {
                monitor.Error( $"Unable to add handler '{h}'. This folder is already handled by {conflict.First()}." );
                return false;
            }
            _resourceHandlers.Insert( 0, h );
        }
        else
        {
            var conflict = _resourceHandlers.Where(
    ...
    public ResourceSpace? Build( IActivityMonitor monitor )
    {
        ...
        var space = new ResourceSpace( packageIndex, _resourceHandlers.ToImmutableArray() );
