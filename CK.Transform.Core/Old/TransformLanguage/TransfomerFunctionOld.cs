...
public sealed class TransfomerFunctionOld : CompositeNode
{
    ...
    static readonly OptionalChild<InjectionPointOld> _onT = new( 4 );
    ...
    TransfomerFunctionOld( ImmutableArray<Trivia> leading, CompositeNodeMutator content, ImmutableArray<Trivia> trailing )
        : base( leading, content, trailing )
    {
    }

    public TransfomerFunctionOld( TokenNode create, TokenNode language, TokenNode transformer, TokenNode? functionName, TokenNode? on, AbstractNode? target, TokenNode? asT, TokenNode begin, NodeList<ITransformStatement> statements, TokenNode end )
    ...
    {
        return new TransfomerFunctionOld( leading, content, trailing );
    }
    ...
    public TransfomerFunctionOld SetName( string? name )
    {
        ...
        else
        {
            ...
            if( on == null )
            {
                mutator.RawItems[_onT.Index] = new TokenNode( TokenType.GenericIdentifier, "on".AsMemory(), Analyzer.OneSpace );
                mutator.RawItems[_functionName.Index] = new TokenNode( TokenType.GenericIdentifier, name.AsMemory(), Analyzer.OneSpace );
            }
            else
            {
        ...
        return (TransfomerFunctionOld)mutator.Clone();
    }

    /// <summary>
    /// Gets the target address or name if it is specified.
    /// <para>
    /// This is either a single-line <see cref="RawStringOld"/> or a <see cref="TokenNode"/> without whitespace.
    /// </para>
    /// </summary>
