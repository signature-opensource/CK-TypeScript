        ...
        public AbstractNode Apply( IActivityMonitor monitor, string? before, string? after, bool clearBlockComments, Trivia[]? triviaReplacement )
        {
            ...
            if( IdxTrivias == null )
            {
                ...
                if( clearBlockComments )
                {
                    e = e.LiftBothTrivias();
                    leading = before != null
                                ? e.LeadingTrivias.Add( new Trivia( NodeType.Whitespace, before ) )
                                : e.LeadingTrivias;
                    trailing = after != null
                                ? e.TrailingTrivias.Insert( 0, new Trivia( NodeType.Whitespace, after ) )
                                : e.TrailingTrivias;
                }
                else
                {
                    leading = before != null
                                ? e.LeadingTrivias.Insert( 0, new Trivia( NodeType.Whitespace, before ) )
                                : e.LeadingTrivias;
                    trailing = after != null
                                ? e.TrailingTrivias.Add( new Trivia( NodeType.Whitespace, after ) )
                                : e.TrailingTrivias;
                }
            ...
            foreach( int idx in IdxTrivias )
            {
                ...
                else
                {
                    if( before != null )
                    {
                        trivias = trivias.Insert( actualIdx++, new Trivia( NodeType.Whitespace, before ) );
                        ++deltaInsert;
                    }
                    if( after != null )
                    {
                        trivias = trivias.Insert( actualIdx + 1, new Trivia( NodeType.Whitespace, after ) );
                        ++deltaInsert;
                    }
                }
